...
atomtype_id     = np.array([  1,    2,     3,     4   ])
atomtype_name   = np.array([ 'C',  'W',   'A',   'B'  ])
atomtype_mass   = np.array([72.0,  72.0,  72.0,  72.0 ] )
atomtype_charge = np.array([ 0.0,  0.0,   1.0,   -1.0  ] )
...
out_h5_filename = 'partition50mM.h5'
out_h5_file     = os.path.join(work_folder, out_h5_filename)
in_gro_file     = os.path.join(work_folder, 'partition50mM.gro')
...
## the key is the msolecule name from the gmx top
