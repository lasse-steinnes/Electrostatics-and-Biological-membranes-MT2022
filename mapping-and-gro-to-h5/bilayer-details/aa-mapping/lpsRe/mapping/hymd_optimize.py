...
def parse_molecule_species(names, molecules=None):
    ...
    species = {}
    for d in (dppc, dmpc, dspc, dopc, popg, toy_lipid, lipid_a, lps_re, ion, w):
    ...
    if isinstance(names, np.ndarray):
        ...
        for m in unique_molecules:
            ...
            print(n)
            print(ion[0][:n_mol])
            print(np.array_equal(n, lipid_a[0][:n_mol]))
            print(np.array_equal(n, ion[0][:n_mol]))
            ...
            elif np.array_equal(n, lps_re[0][:n_mol]) and n_mol == 33:
