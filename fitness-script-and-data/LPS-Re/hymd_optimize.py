...
def parse_molecule_species(names, molecules=None):
    ...
    species = {}
    for d in (dppc, dmpc, dspc, dopc, popg, toy_lipid, lipid_a, lre, ion, w):
        ...
        for s in species_:
            ...
            if s in ("all", "solvent", "ions"):
                if s == "all":
                    ...
                    species_["ions"] = (
                        names.atoms.select_atoms("type N").indices
                    )
            else:
                species_[s] = (
...
def plot_histogram(
    ...
    for s in keys:
        ...
        if vmd_colors and "type" in s:
            if s == "type:N":
                ...
                color = "grey" #  (0.25, 0.75, 0.75)  # cyan
                label = "C"
                marker = "x"
            elif s == "type:P":
                color = "purple" # (0.5, 0.5, 0.2)  # tan
                label = "P"
                marker = "^"
            elif s == "type:G":
                color = "green" #(1.0, 0.6, 0.6)  # pink
                label = "G"
                marker = "v"
            elif s == "type:W":
                color = "cyan" #(0.0, 0.0, 0.0)  # black
                label = "W"
                marker = "s"
            elif s == "type:D":
                color = (0.3, 0.8, 0.8)
        ...
        ax.plot(
            ...
            marker=marker, markersize = 10, markevery = 5, linewidth = 3
        )
    ...
    if not remove_legend:
        ax.legend(loc="center right", fontsize=fontsize, framealpha=0)
    ...
    if show:
        plt.savefig("./density-profile.pdf",format = "pdf")
        plt.show()
